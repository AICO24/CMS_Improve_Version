document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Phase 7 (ported from Phase 6A): floating widget open/close only —
    // purely presentational, the chat DOM/state above is untouched by
    // opening or closing, so the conversation is preserved across toggles.
    ...
    chatFindLotsBtn.addEventListener('click', async function() {
        ...
        await withButtonLoading(chatFindLotsBtn, async () => {
            ...
            showStep(2);
            // Phase 7: collapse the floating panel so it doesn't cover the
            // recommendation cards it just triggered (same fix as Phase 6A).
            setChatPanelOpen(false);
        });
    });
